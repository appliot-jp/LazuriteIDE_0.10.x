...
static byte check_crc(uint16_t message_from_sensor, uint8_t check_value_from_sensor);
static float read_value(byte cmd);
...
static float read_value(byte cmd)
{
	...
	Wire.write_byte(cmd); //Measure value (prefer no hold!)
	...
	//Hang out while measurement is taken. datasheet says 50ms, practice may call for more
        for (counter = 0, num_read = 0; counter < MAX_COUNTER && num_read != 3; counter++) {
	        delay(DELAY_INTERVAL);
                ...
                num_read = Wire.requestFrom(HTDU21D_ADDRESS, 3,true);
        }
	...
	raw_value = ((unsigned int) msb << 8) | (unsigned int) lsb;
...
static void setResolution(byte resolution)
{
...
static byte readUserRegister(void)
{
  ...
  Wire.beginTransmission(HTDU21D_ADDRESS);
  Wire.write_byte(READ_USER_REG); //Read the user register
  Wire.endTransmission(false);
  
  //Read result
  Wire.requestFrom(HTDU21D_ADDRESS, 1,true);
...
{
  Wire.beginTransmission(HTDU21D_ADDRESS);
  Wire.write_byte(WRITE_USER_REG); //Write to the user register
  Wire.write_byte(val); //Write the new resolution bits
...
static byte check_crc(uint16_t message_from_sensor, uint8_t check_value_from_sensor)
{
  ...
  //message = 0x4E85, checkvalue is 0x6B
	uint32_t remainder;
	uint32_t divsor;
  remainder = (uint32_t)message_from_sensor << 8; //Pad with 8 bits because we have to add in the check value
