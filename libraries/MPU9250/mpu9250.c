...
void writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
	...
	Wire.write_byte(subAddress);           // Put slave register address in Tx buffer
	Wire.write_byte(data);                 // Put data in Tx buffer
...
//	Wire.requestFrom(address, 1);  // Read one byte from slave register address 
	Wire.requestFrom(address, (size_t) 1,true);  // Read one byte from slave register address 
	data = (uint8_t)Wire.read();                      // Fill Rx buffer with result
...
//  writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; a rate consistent with the filter update rate 
  writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 99);  // Use a 200 Hz rate; a rate consistent with the filter update rate 
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
	...
	uint8_t selfTest[6];
	int16_t gAvg[3]={0,0,0}, aAvg[3]={0,0,0}, aSTAvg[3]={0,0,0}, gSTAvg[3]={0,0,0};
	float factoryTrim[6];
	uint8_t FS = 0;
	int ii;
