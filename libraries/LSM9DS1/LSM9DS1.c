...
#define LSM9DS1_WHO_AM_I_M_VAL		( 0x3C )
...
/*#                                Pototype                                  #*/
/*############################################################################*/
bool lsm9ds1_availableGyro(void);
void lsm9ds1_getGyro(float *val);
void lsm9ds1_powerDownAccGyro(void);
...
static void lsm9ds1_init(uint8_t op_mode, uint8_t i2c_addr_xlg, uint8_t i2c_addr_m)
{
	...
	if (op_mode & LSM9DS1_OPMODE_XLG) {
		_slave_addr_xlg = (i2c_addr_xlg == LSM9DS1_I2C_ADDR_XLG_6A) ? LSM9DS1_I2C_ADDR_XLG_6A : LSM9DS1_I2C_ADDR_XLG_6B;
		res = _i2c_read(LSM9DS1_WHO_AM_I, &data, 1);
		if ((res == I2C_RESULT_OK) && (data == LSM9DS1_WHO_AM_I_VAL)) {
			_acc_gyro_mode = true;
		}
	} else if (op_mode & LSM9DS1_OPMODE_XL) {
		_slave_addr_xlg = (i2c_addr_xlg == LSM9DS1_I2C_ADDR_XLG_6A) ? LSM9DS1_I2C_ADDR_XLG_6A : LSM9DS1_I2C_ADDR_XLG_6A;
		res = _i2c_read(LSM9DS1_WHO_AM_I, &data, 1);
		if ((res == I2C_RESULT_OK) && (data == LSM9DS1_WHO_AM_I_VAL)) {
			_acc_only_mode = true;
		}
 	...
 	if (op_mode & LSM9DS1_OPMODE_MAG) {
		_slave_addr_m = (i2c_addr_m == LSM9DS1_I2C_ADDR_M_1C) ? LSM9DS1_I2C_ADDR_M_1C : LSM9DS1_I2C_ADDR_M_1E;
...
// for debug purpose only
/* static uint8_t lsm9ds1_reg_read(uint8_t dev, uint8_t addr)
{
	uint8_t data;
