...
static byte IIC_Read(byte regAddr)
{
  // This function reads one byte over IIC
  Wire.beginTransmission(MPL3115A2_ADDRESS);
  Wire.write_byte(regAddr);  // Address of CTRL_REG1
  Wire.endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
  Wire.requestFrom(MPL3115A2_ADDRESS, 1,true); // Request the data...
  return Wire.read();
}

static void IIC_Write(byte regAddr, byte value)
{
  // This function writes one byto over IIC
  Wire.beginTransmission(MPL3115A2_ADDRESS);
  Wire.write_byte(regAddr);
...
static void setOversampleRate(byte sampleRate)
{
	byte tempSetting ;
...
static float readAltitude()
{
	...
	Wire.beginTransmission(MPL3115A2_ADDRESS);
	Wire.write_byte(OUT_P_MSB);  // Address of data to get
	Wire.endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
	if (Wire.requestFrom(MPL3115A2_ADDRESS, 3,true) != 3) { // Request three bytes
		return -999;
	}
...
{
	int counter;
	float pressure_decimal; //Turn it into fraction
	float pressure;
	byte msb, csb, lsb;
	long pressure_whole;
...
static float readTemp()
{
	...
	word foo;
    bool negSign;
	float templsb; //temp, fraction of a degree
	float temperature;
	...
	Wire.beginTransmission(MPL3115A2_ADDRESS);
	Wire.write_byte(OUT_T_MSB);  // Address of data to get
	Wire.endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
	if (Wire.requestFrom(MPL3115A2_ADDRESS, 2,true) != 2) { // Request two bytes
		return -999;
	}
	...
	// shift the value over 4 spots to the right and divide by 16 (since 
	// there are 16 values in 4-bits). 
	templsb = (lsb>>4)/16.0; //temp, fraction of a degree
...
const MPL3115A2 mpl3115a2 =
{
	...
	setModeBarometer,		// Puts the sensor into Pascal measurement mode.
	setModeAltimeter,		// Puts the sensor into altimetery mode.
	setModeStandby,			// Puts the sensor into Standby mode. Required when changing CTRL1 register.
	setModeActive,			// Start taking measurements!
	setOversampleRate,		// Sets the # of samples from 1 to 128. See datasheet.
	enableEventFlags		// Sets the fundamental event flags. Required during setup.
};
